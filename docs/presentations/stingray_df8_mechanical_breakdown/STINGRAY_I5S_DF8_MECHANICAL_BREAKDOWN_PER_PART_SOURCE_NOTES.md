...
This package contains the complete deterministic per-part PNG render set, manifests, source facts, context views, and 73-slide `@oai/artifact-tool` PowerPoint authoring source. The final PPTX export is not present because the required workspace-dependency loader/runtime was unavailable in this execution environment.
...
- PowerPoint export status: **BLOCKED — `load_workspace_dependencies` and its required `@oai/artifact-tool` runtime paths were unavailable; no prohibited fallback authoring engine was used.**
